...
### 31) Feature-flagged cross-DOCFORMAT Y carry to reduce CASE-to-CASE vertical drift
- Added `XEROX_ENABLE_CROSS_DOCFORMAT_Y` (default OFF) to keep baseline behavior stable for broad sample sets.
- New anchor classifier per PREFIX DOCFORMAT:
  - `absolute`: case contains `MOVETO`, `SETLKF`, or `SETPAGEDEF`
  - `reset`: case contains `PAGEBRK`, `NEWFRONT`, or `NEWBACK`
  - `carry`: continuation-style case (no explicit anchor/reset)
- When flag is ON:
  - carry cases start OUTLINE at `POSITION LEFT (FLOW_Y)` to reuse previous case Y
  - absolute cases keep `POSITION LEFT NEXT`
  - reset cases keep `POSITION LEFT SAME`
  - closing an OUTLINE stores `FLOW_Y = $SL_LMAXY` and marks `FLOW_VALID = 1`
  - `FLOW_VALID` is reset on `PAGEBRK`, `NEWFRONT`, `NEWBACK`, and `SETLKF`
- Validation run (DocEXEC RC 0):
  - CASIO: `C:\ISIS\samples_pdd\OCBC\TEST_CASIO_CROSS\afpds\CASIO.pdf`
  - SIBS_CAST: `C:\ISIS\samples_pdd\OCBC\TEST_SIBS_CROSS\afpds\SIBS_CAST.pdf`
- Anchor decision report generated: `anchor_decisions_report.md` (class and first `POSITION LEFT` mapping per DOCFORMAT).
